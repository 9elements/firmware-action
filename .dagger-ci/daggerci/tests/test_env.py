...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["NONEXISTING_A", "NONEXISTING_B"],
            "/tmp",
            [["NONEXISTING_A", "/home"], ["NONEXISTING_B", "/usr"]],
            "/home",
        ),
    ],
)
def test__get_env_var_value(
    monkeypatch, list_of_maybe_existing_env_vars, fallback, presets, expected
):
    # Prepare environment
    for pair in presets:
    ...
    assert (
        get_env_var_value(
            list_of_maybe_existing_env_vars=list_of_maybe_existing_env_vars,
            fallback=fallback,
        )
        == expected
    )
