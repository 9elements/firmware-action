...
func TestExecuteUpToDate(t *testing.T) {
	...
	timestampFile := filepath.Join(TimestampsDir, FilenameForFirmwareModule(target, "txt"))
	...
	configFile := filepath.Join(CompiledConfigsDir, FilenameForFirmwareModule(target, "json"))
